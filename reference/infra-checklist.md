...
`agents/infra-auditor.md`; consult this for the specifics. A consuming project's
CLAUDE.md overrides anything here.

Tiers stay exposure-gated: no path from an attacker or an outage to the resource means
the finding is `basis: inferred` and drops a tier.
...
If the tool can't be determined, say so in `coverage` and rate defaults-dependent
findings `basis: inferred`.
