...
specifics. A consuming project's CLAUDE.md overrides anything here.

Tiers stay impact-gated: no path from the failure to user or operator impact means the
finding is `basis: inferred` and drops a tier.
...
The deploy manifest's grace period (the infrastructure lane's) only helps if the
application uses it:
