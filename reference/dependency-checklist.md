...
codebase to the vulnerable API means the finding is `basis: inferred` and drops a tier.
...
- **Degrade path:** if every route above fails (offline gate run, blocked egress),
  say "could not verify — advisory data unreachable" in `coverage` and mark
  affected findings `basis: inferred`. Never imply clean, and never guess a CVE id you cannot
