...
The standard the prompt lane judges against — **lookup data** for reviewing other
people's prompts. Not a detection crutch: a capable model already knows these defect
classes, and this is the specifics it will not recall verbatim — per-dimension probe
lists, the prompt-surface signature table per ecosystem, and token-economy heuristics.
A consuming project's documented prompt conventions override
...
A repo matching no row has no prompt surface, and the lane self-skips: an empty
findings list with a coverage line saying so.
