...
including its linters' performance-class findings. **Simplicity is not yours on a
changeset**: reuse of an existing helper, a wrapper over a wrapper, a fix left in the
caller, scaffold standing in for one function. exorcist owns those, and fixes what it
finds.
